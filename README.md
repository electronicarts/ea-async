...
Start your application with an extra JVM parameter: `-javaagent:ea-async-1.0.6.jar`
```
 java -javaagent:ea-async-1.0.6.jar -cp your_claspath YourMainClass args...
```
...
The ea-async-1.0.6.jar is a runnable jar that can pre-instrument your files.

Usage:

```bash
java -cp YOUR_PROJECT_CLASSPATH -jar ea-async-1.0.6.jar classDirectory
```

Example:

```bash
java -cp guava.jar;commons-lang.jar  -jar ea-async-1.0.6.jar target/classes
```
